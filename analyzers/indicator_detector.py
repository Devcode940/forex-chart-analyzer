...
class IndicatorDetector:
    ...
    def _extract_line_points(self, mask: np.ndarray) -> list:
        ...
        # Vectorized slice processing using 3D matrix reshape & reduction
        trimmed_w = num_slices * slice_width
        sub_mask = mask[:, :trimmed_w]
        reshaped = sub_mask.reshape(h, num_slices, slice_width)

        # Active pixel count per y-row in each slice
        slice_counts = (reshaped > 0).sum(axis=2)
        slice_totals = slice_counts.sum(axis=0)

        y_indices = np.arange(h)
        y_sums = np.dot(y_indices, slice_counts)

        valid = slice_totals > 0
        valid_slices = np.where(valid)[0]

        center_y = (y_sums[valid] / slice_totals[valid]).astype(int)
        center_x = (valid_slices * slice_width + (valid_slices + 1) * slice_width) // 2

        return [{"x": int(x), "y": int(y)} for x, y in zip(center_x, center_y)]
