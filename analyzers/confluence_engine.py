...
class ConfluenceEngine:
    ...
    def _collect_fib_signals(self, fib: dict, sltp: dict):
        ...
        if golden_zone:
            ...
            in_zone = (
                lower <= current <= upper if current > 0 and upper > lower else True
            )
            if in_zone:
                self._add_signal(
                    "Fibonacci",
        ...
        for ratio, level in fib.get("retracements", {}).items():
            if ratio in [0.618, 0.786] and level.get("importance") == "HIGH":
                ...
                proximity = (
                    abs(fib_value - current) / (current + 1e-6) if current > 0 else 1.0
                )
                if proximity < 0.05:
                    direction = (
