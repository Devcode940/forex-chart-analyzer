...
class MLAnomalyDetector:
    ...
    def detect(self, feature_vector: np.ndarray, price_series: dict) -> dict:
        """
        ...
        z_scores = np.abs(
            (current_scaled - self.scaler.mean_) / (self.scaler.scale_ + 1e-8)
        )
        ...
        return {
            ...
            "methods": {
                ...
                "pca_reconstruction": {
                    ...
                    "explained_variance": round(
                        float(np.sum(self.pca.explained_variance_ratio_)), 3
                    ),
                },
                "zscore_analysis": {
            ...
            "interpretation": self._interpret_anomaly(
                level, composite, iso_label, n_extreme
            ),
    ...
    def _generate_baseline_data(self, n_samples: int = 1000) -> np.ndarray:
        ...
        for i in range(n_samples):
            regime = np.random.choice(
                ["trend", "range", "volatile"], p=[0.5, 0.35, 0.15]
            )
            ...
            X[i, 14:20] = [sigma * 10, X[i, 10] / (X[i, 12] + 1e-8)] + [
                np.random.uniform(0.1, 0.5) for _ in range(4)
            ]
            X[i, 20:30] = [abs(mu) / (sigma + 1e-8) * 0.3, mu / (sigma + 1e-8)] + [
