...
class StatisticalValidator:
    ...
    def monte_carlo_validation(
        ...
        # ⚡ OPTIMIZATION: Fully vectorized Geometric Brownian Motion path simulation and validation.
        # This eliminates the slow python nested loops, running thousands of simulations in milliseconds.
        if pattern_direction != "PENDING" and n_simulations > 0:
            # Generate a 2D matrix of random returns: shape (n_simulations, n_bars - 1)
            random_returns = np.random.normal(mu, sigma, (n_simulations, n_bars - 1))
            # Pre-allocate 2D matrix for random prices
            random_prices = np.zeros((n_simulations, n_bars))
            random_prices[:, 0] = smoothed[0]
            # Vectorized cumulative product across columns
            random_prices[:, 1:] = smoothed[0] * np.cumprod(1 + random_returns, axis=1)

            mid_point = n_bars // 2
            # Vectorized percentage changes for the second half of each simulation
    ...
    def bootstrap_confidence_interval(
        ...
        if n_bootstrap <= 0 or n <= 0:
            return {"error": "Invalid data or parameters for bootstrap"}

        # ⚡ OPTIMIZATION: Fully vectorized Bootstrap resampling using 2D NumPy operations.
        # Avoids 10,000 slow sequential iterations and computes standard metrics in parallel.
        # Generates a 2D index matrix of shape (n_bootstrap, n)
        ...
        # 1. R2 trend strength calculation (vectorized Pearson correlation squared)
        # Avoids np.polyfit in loops. For simple linear regression, R2 is equivalent to r^2.
        x = np.arange(n)
        x_mean = np.mean(x)
        x_dev = x - x_mean
        ...
        covariance = np.dot(y_dev, x_dev)
        trend_strengths = np.zeros(n_bootstrap)
        valid_mask = y_var > 0
        trend_strengths[valid_mask] = (covariance[valid_mask] ** 2) / (
            x_var * y_var[valid_mask]
        )
        ...
        wins = np.sum(np.sign(samples) == direction[:, np.newaxis], axis=1)
        win_rates = wins / n
    ...
    def _calc_trend_r2(self, prices: np.ndarray) -> float:
        """R² from linear regression (kept for backwards compatibility)."""
