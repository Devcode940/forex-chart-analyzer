...
class StatisticalValidator:
    ...
    def monte_carlo_validation(
        ...
        if n_simulations > 0 and n_bars > 1:
            ...
            random_returns_all = np.random.normal(
                mu, sigma, (n_simulations, n_bars - 1)
            )
            random_prices = np.zeros((n_simulations, n_bars))
            random_prices[:, 0] = smoothed[0]
            random_prices[:, 1:] = (
                np.cumprod(1 + random_returns_all, axis=1) * smoothed[0]
            )
            ...
            if pattern_direction != "PENDING":
                mid_point = n_bars // 2
                random_changes = (
                    random_prices[:, -1] - random_prices[:, mid_point]
    ...
    def bootstrap_confidence_interval(
        ...
        if n_bootstrap > 0 and n > 0:
            ...
            reconstructed_all = (
                np.cumprod(1 + sample_all, axis=1) * smoothed[0]
            )  # shape: (n_bootstrap, n)
            ...
            directions_all = np.where(
                reconstructed_all[:, -1] > reconstructed_all[:, 0], 1, -1
            )[:, np.newaxis]
