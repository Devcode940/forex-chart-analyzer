...
class StatisticalValidator:
    ...
    def monte_carlo_validation(
        ...
        # Generate 2D array of random returns (size: n_simulations, n_bars - 1)
        # We completely vectorize the Monte Carlo path generation over 2D NumPy matrices
        ...
        # Reconstruct paths using cumulative product along axis 1
    ...
    def bootstrap_confidence_interval(
        ...
        # Bootstrap resampling using 2D matrix operations to eliminate slow nested loops
        n = len(returns)
        # Generate all resampled indices at once: shape (n_bootstrap, n)
        sample_indices = np.random.randint(0, n, size=(n_bootstrap, n))
        samples = returns[sample_indices]  # shape (n_bootstrap, n)

        # Reconstruct paths for all resamples: shape (n_bootstrap, n)
        ...
        # 1. Trend R² scores computed using vectorized Pearson correlation squared (algebraic OLS) formula
        x = np.arange(n)
        mean_x = np.mean(x)
        x_diff = x - mean_x
        denom_x = np.sum(x_diff ** 2)
        mean_y = np.mean(reconstructed_paths, axis=1, keepdims=True)
        y_diff = reconstructed_paths - mean_y
        cov_xy = np.dot(y_diff, x_diff)
        ss_tot_vec = np.sum(y_diff ** 2, axis=1)
        trend_strengths = np.where(ss_tot_vec > 0, (cov_xy ** 2) / (denom_x * ss_tot_vec), 0.0)

        # 2. Volatilities: shape (n_bootstrap,)
        volatilities = np.std(samples, axis=1)

        # 3. Efficiency Ratios: shape (n_bootstrap,)
        ...
        # 4. Win rates: shape (n_bootstrap,)
        directions_vec = np.where(reconstructed_paths[:, -1] > reconstructed_paths[:, 0], 1, -1)
        matching_vec = (np.sign(samples) == directions_vec[:, np.newaxis])
        win_rates = np.sum(matching_vec, axis=1) / n
