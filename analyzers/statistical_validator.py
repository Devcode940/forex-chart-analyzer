...
class StatisticalValidator:
    ...
    def bootstrap_confidence_interval(
        ...
        # Bulk bootstrap resampling using NumPy vectorization
        # Sample returns with replacement in bulk: shape (n_bootstrap, n)
        sample_indices = np.random.randint(0, n, size=(n_bootstrap, n))
        samples = returns[sample_indices]

        # Reconstruct price paths in bulk: shape (n_bootstrap, n)
        # Note: reconstructed length is strictly n (matching original definition)
        reconstructed_paths = np.cumprod(1 + samples, axis=1) * smoothed[0]

        # 1. Vectorized Trend R² calculation
        # Since x is identical for every reconstructed path: x = np.arange(n)
        x_arr = np.arange(n)
        ...
        denom = x_var * y_var
        trend_strengths = np.where(denom > 0, (cov**2) / denom, 0.0)
        ...
        efficiency_ratios = np.where(path_diffs > 0, net_vec / path_diffs, 0.0)

        # 4. Vectorized Win Rates
        direction_vec = np.where(
            reconstructed_paths[:, -1] > reconstructed_paths[:, 0], 1, -1
        )[:, np.newaxis]
        win_rates = np.sum(np.sign(samples) == direction_vec, axis=1) / n
