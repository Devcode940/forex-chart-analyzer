...
class StatisticalValidator:
    ...
    def bootstrap_confidence_interval(
        ...
        # Generate a 2D array of resampled returns indices in a single operation
        sample_indices = np.random.randint(0, n, size=(n_bootstrap, n))
        samples = returns[sample_indices]

        # Reconstruct price paths (each row is a path)
        reconstructed_paths = np.cumprod(1 + samples, axis=1) * smoothed[0]

        # 1. Volatilities: standard deviation along rows
        volatilities = np.std(samples, axis=1)

        # 2. Efficiency Ratios: net move divided by total path length
        net_moves = np.abs(reconstructed_paths[:, -1] - reconstructed_paths[:, 0])
        total_paths = np.sum(np.abs(np.diff(reconstructed_paths, axis=1)), axis=1)
        efficiency_ratios = np.where(total_paths > 0, net_moves / total_paths, 0.0)

        # 3. Win Rates: fraction of bars going in the trend direction
        directions = np.where(
            reconstructed_paths[:, -1] > reconstructed_paths[:, 0], 1, -1
        )
        sample_signs = np.sign(samples)
        wins = np.sum(sample_signs == directions[:, np.newaxis], axis=1)
        win_rates = wins / n

        # 4. Trend Strengths: vectorized algebraic Ordinary Least Squares (OLS)
        x = np.arange(n)
        x_mean = (n - 1) / 2.0
        x_dev = x - x_mean
        ss_x = n * (n * n - 1) / 12.0
        reconstructed_means = np.mean(reconstructed_paths, axis=1)
        cov_xy = np.dot(reconstructed_paths, x_dev)
        # Using np.einsum for fast row-wise dot products (squares) of reconstructed paths
        row_squares = np.einsum("ij,ij->i", reconstructed_paths, reconstructed_paths)
        denom = ss_x * (row_squares - n * (reconstructed_means**2))

        trend_strengths = np.zeros(n_bootstrap)
        valid = denom > 1e-12
        trend_strengths[valid] = (cov_xy[valid] ** 2) / denom[valid]

        # Convert back to native Python lists for compatibility with upstream statistics
    ...
    def _calc_trend_r2(self, prices: np.ndarray) -> float:
        """R² from linear regression calculated via algebraic OLS (bypassing np.polyfit)."""
        n = len(prices)
        if n < 2:
            return 0.0
        x = np.arange(n)
        ...
        cov_xy = np.dot(prices, x_dev)
        denom = ss_x * (np.dot(prices, prices) - n * (y_mean**2))
        return (cov_xy**2) / denom if denom > 1e-12 else 0.0
