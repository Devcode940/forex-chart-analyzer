...
class StatisticalValidator:
    ...
    def monte_carlo_validation(
        ...
        # Vectorized implementation of Monte Carlo simulation to bypass slow Python loops
        if pattern_direction != "PENDING" and n_simulations > 0:
            random_returns_sims = np.random.normal(
                mu, sigma, (n_simulations, n_bars - 1)
            )
            random_prices_matrix = np.zeros((n_simulations, n_bars))
            random_prices_matrix[:, 0] = smoothed[0]
            random_prices_matrix[:, 1:] = smoothed[0] * np.cumprod(
                1 + random_returns_sims, axis=1
            )

            mid_point = n_bars // 2
            random_changes = (
                random_prices_matrix[:, -1] - random_prices_matrix[:, mid_point]
            ) / (random_prices_matrix[:, mid_point] + 1e-6)
    ...
    def bootstrap_confidence_interval(
        ...
        # Vectorized Bootstrap resampling using 2D NumPy matrices for ultra-fast execution
        n = len(returns)
        if n_bootstrap > 0 and n > 0:
            # Resample returns with replacement
            sample_idxs = np.random.randint(0, n, size=(n_bootstrap, n))
            samples = returns[sample_idxs]

            # Reconstruct price paths
            reconstructed_paths = np.cumprod(1 + samples, axis=1) * smoothed[0]

            # 1. Trend strength (R²) using vectorized Ordinary Least Squares formulas
            x = np.arange(n)
            x_mean = np.mean(x)
            x_dev = x - x_mean
            x_var = np.sum(x_dev**2)

            y_mean = np.mean(reconstructed_paths, axis=1, keepdims=True)
            y_dev = reconstructed_paths - y_mean
            y_var = np.sum(y_dev**2, axis=1)
            cov = np.dot(y_dev, x_dev)
            ...
            path_diffs = np.sum(np.abs(np.diff(reconstructed_paths, axis=1)), axis=1)
            efficiency_ratios = np.where(path_diffs > 0, net / path_diffs, 0.0)
