...
class StatisticalValidator:
    ...
    def bootstrap_confidence_interval(
        ...
        net_vectorized = np.abs(
            reconstructed_matrix[:, -1] - reconstructed_matrix[:, 0]
        )
        path_vectorized = np.sum(np.abs(np.diff(reconstructed_matrix, axis=1)), axis=1)
        efficiency_ratios = np.where(
            path_vectorized > 0, net_vectorized / path_vectorized, 0.0
        )

        # 6. Vectorized win rates
        direction_vec = np.where(
            reconstructed_matrix[:, -1] > reconstructed_matrix[:, 0], 1, -1
        )[:, np.newaxis]
