...
class StatisticalValidator:
    ...
    def monte_carlo_validation(
        ...
        # Bolt optimization: Vectorized Monte Carlo Geometric Brownian Motion path generation across all simulations
        if n_simulations > 0:
            sim_returns = np.random.normal(mu, sigma, (n_simulations, n_bars - 1))
            cum_factors = np.cumprod(1 + sim_returns, axis=1)
            random_prices = np.empty((n_simulations, n_bars))
            random_prices[:, 0] = smoothed[0]
            random_prices[:, 1:] = smoothed[0] * cum_factors
    ...
    def bootstrap_confidence_interval(
        ...
        # Bolt optimization: Fully vectorized 2D matrix operations for bootstrap resampling
        n = len(returns)

        # 2D sample index selection: shape (n_bootstrap, n)
        sample_idx = np.random.randint(0, n, size=(n_bootstrap, n))
        samples = returns[sample_idx]

        # Reconstruct price paths across all resamples: shape (n_bootstrap, n)
        reconstructed = np.cumprod(1 + samples, axis=1) * smoothed[0]

        # Volatility metric across resamples
        volatilities = np.std(samples, axis=1)

        # Efficiency ratio metric across resamples
        ...
        # Win rate metric across resamples
        ...
        # Trend strength (R²) via vectorized OLS Pearson correlation over 2D matrix
        x_dev = np.arange(n) - (n - 1) / 2.0
        var_x = np.sum(x_dev**2)
        y_mean = np.mean(reconstructed, axis=1, keepdims=True)
        y_dev = reconstructed - y_mean
        cov = np.dot(y_dev, x_dev)
        var_y = np.sum(y_dev**2, axis=1)
        trend_strengths = np.where(var_y > 0, (cov**2) / (var_x * var_y), 0)
