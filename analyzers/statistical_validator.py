...
class StatisticalValidator:
    ...
    def bootstrap_confidence_interval(
        ...
        Entirely vectorized using 2D NumPy operations to avoid heavy sequential loops and polyfit calls.
        """
        smoothed = np.array(price_series.get("smoothed", []))
        ...
        # Vectorized generation of resampled indices and returns: shape (n_bootstrap, n)
        sample_idx = np.random.randint(0, n, size=(n_bootstrap, n))
        samples = returns[sample_idx]

        # Reconstruct price paths for all simulations: shape (n_bootstrap, n)
        reconstructed_paths = np.cumprod(1 + samples, axis=1) * smoothed[0]

        # Trend R2 calculated via vectorized Pearson correlation squared over 2D matrix
        x = np.arange(n)
        x_mean = np.mean(x)
        x_dev = x - x_mean
        ...
        ss_yy = np.sum(y_dev**2, axis=1)
        ss_xy = np.dot(y_dev, x_dev)
        trend_strengths = np.where(ss_yy > 0, (ss_xy**2) / (ss_xx * ss_yy + 1e-12), 0.0)

        # Vectorized volatilities: shape (n_bootstrap,)
        volatilities = np.std(samples, axis=1)

        # Vectorized efficiency ratios
        ...
        # Vectorized win rates
        direction = np.where(
            reconstructed_paths[:, -1] > reconstructed_paths[:, 0], 1, -1
        )
        direction_col = direction[:, np.newaxis]
        wins = np.sum(np.sign(samples) == direction_col, axis=1)
        win_rates = wins / n
