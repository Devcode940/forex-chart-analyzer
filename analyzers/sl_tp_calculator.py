...
class SLTPCalculator:
    ...
    def calculate(
        ...
        self._structure_based_scenarios(
            structure_results, current_price, price_range, smoothed
        )
    ...
    def _pattern_based_scenarios(
        self, patterns: list, current_price: float, price_range: float
    ):
    ...
    def _sr_based_scenarios(
        self, sr: dict, current_price: float, price_range: float, bias: str
    ):
        ...
        if bias in ["BULLISH", "NEUTRAL"] and nearest_support:
            ...
            tp = (
                ...
                else current_price + (current_price - sl) * 2
            )
            self.recommendations.append(
                {
        ...
        if bias in ["BEARISH", "NEUTRAL"] and nearest_resistance:
            ...
            tp = (
                nearest_support
                if nearest_support
                else current_price - (sl - current_price) * 2
            )
            self.recommendations.append(
                {
    ...
    def _structure_based_scenarios(
        self,
        structure: dict,
        current_price: float,
        price_range: float,
        ...
        if bullish_breaks:
            recent_low = (
                min([sl["value"] for sl in swing_lows[-2:]])
                if len(swing_lows) >= 2
                else current_price - price_range * 0.1
            )
            recent_high = (
                max([sh["value"] for sh in swing_highs[-2:]])
                if len(swing_highs) >= 2
                else current_price + price_range * 0.3
            )
            self.recommendations.append(
                {
    ...
    def _atr_based_scenarios(
        self, smoothed: np.ndarray, current_price: float, bias: str
    ):
    ...
    def _conservative_scenario(
        self, bias: str, current_price: float, price_range: float
    ):
