...
class CandlestickDetector:
    ...
    def _detect_doji(self, candles: list):
        ...
        for c in candles:
            ...
            if body_ratio < 0.1:
                ...
                self._add_pattern(
                    "Doji", "indecision", "REVERSAL_POSSIBLE",
                    c["index"], c["x"], conf,
    ...
    def _detect_hammer(self, candles: list):
        ...
        for i, c in enumerate(candles):
            ...
            if (c["lower_wick"] > c["body"] * 2 and
                c["upper_wick"] < c["body"] * 0.5 and
                c["body_ratio"] < 0.4):
                # Context: better after downtrend
                context_bonus = 0.15 if i > 2 and candles[i-2]["is_bullish"] is False else 0
                self._add_pattern(
                    "Hammer", "bullish_reversal", "BUY",
                    c["index"], c["x"], 0.7 + context_bonus,
    ...
    def _detect_inverted_hammer(self, candles: list):
        ...
        for i, c in enumerate(candles):
            if c["range"] < 1e-6:
                continue
            if (c["upper_wick"] > c["body"] * 2 and
                c["lower_wick"] < c["body"] * 0.5 and
                c["body_ratio"] < 0.4 and
                not c["is_bullish"]):
                self._add_pattern(
                    "Inverted Hammer", "bullish_reversal", "BUY",
                    c["index"], c["x"], 0.6,
    ...
    def _detect_shooting_star(self, candles: list):
        ...
        for i, c in enumerate(candles):
            ...
            if (c["upper_wick"] > c["body"] * 2 and
                c["lower_wick"] < c["body"] * 0.5 and
                c["is_bullish"] is False):
                context_bonus = 0.15 if i > 2 and candles[i-2].get("is_bullish", False) else 0
                self._add_pattern(
                    "Shooting Star", "bearish_reversal", "SELL",
                    c["index"], c["x"], 0.7 + context_bonus,
    ...
    def _detect_bullish_engulfing(self, candles: list):
        ...
        for i in range(1, len(candles)):
            ...
            if (not prev["is_bullish"] and curr["is_bullish"] and
                ...
                curr["body"] > prev["body"]):
                self._add_pattern(
                    "Bullish Engulfing", "bullish_reversal", "BUY",
                    curr["index"], curr["x"], 0.75,
    ...
    def _detect_bearish_engulfing(self, candles: list):
        ...
        for i in range(1, len(candles)):
            ...
            if (prev["is_bullish"] and not curr["is_bullish"] and
                ...
                curr["body"] > prev["body"]):
                self._add_pattern(
                    "Bearish Engulfing", "bearish_reversal", "SELL",
                    curr["index"], curr["x"], 0.75,
    ...
    def _detect_morning_star(self, candles: list):
        ...
        for i in range(2, len(candles)):
            ...
            if (not first["is_bullish"] and
                second["body_ratio"] < 0.3 and
                third["is_bullish"] and
                third["close"] > (first["open"] + first["close"]) / 2):
                self._add_pattern(
                    "Morning Star", "bullish_reversal", "BUY",
                    third["index"], third["x"], 0.8,
    ...
    def _detect_evening_star(self, candles: list):
        ...
        for i in range(2, len(candles)):
            ...
            if (first["is_bullish"] and
                ...
                third["close"] < (first["open"] + first["close"]) / 2):
                self._add_pattern(
                    "Evening Star", "bearish_reversal", "SELL",
                    third["index"], third["x"], 0.8,
    ...
    def _detect_pin_bar_bull(self, candles: list):
        ...
        for c in candles:
            ...
            if wick_ratio > 0.65 and c["body_ratio"] < 0.25 and c["is_bullish"]:
                self._add_pattern(
                    "Bullish Pin Bar", "bullish_reversal", "BUY",
                    c["index"], c["x"], 0.75,
    ...
    def _detect_pin_bar_bear(self, candles: list):
        ...
        for c in candles:
            ...
            if wick_ratio > 0.65 and c["body_ratio"] < 0.25 and not c["is_bullish"]:
                self._add_pattern(
                    "Bearish Pin Bar", "bearish_reversal", "SELL",
                    c["index"], c["x"], 0.75,
    ...
    def _detect_three_white_soldiers(self, candles: list):
        ...
        for i in range(2, len(candles)):
            ...
            if (first["is_bullish"] and second["is_bullish"] and third["is_bullish"] and
                ...
                self._add_pattern(
                    "Three White Soldiers", "bullish_continuation", "BUY",
                    third["index"], third["x"], 0.8,
    ...
    def _detect_three_black_crows(self, candles: list):
        ...
        for i in range(2, len(candles)):
            ...
            if (not first["is_bullish"] and not second["is_bullish"] and not third["is_bullish"] and
                second["open"] <= first["open"] and second["close"] < first["close"] and
                third["open"] <= second["open"] and third["close"] < second["close"]):
                self._add_pattern(
                    "Three Black Crows", "bearish_continuation", "SELL",
                    third["index"], third["x"], 0.8,
    ...
    def _detect_marubozu_bull(self, candles: list):
        ...
        for c in candles:
            ...
            if c["is_bullish"] and c["body_ratio"] > 0.85 and wick_total < 0.15:
                self._add_pattern(
                    "Bullish Marubozu", "bullish_continuation", "BUY",
                    c["index"], c["x"], 0.7,
    ...
    def _detect_marubozu_bear(self, candles: list):
        ...
        for c in candles:
            ...
            if not c["is_bullish"] and c["body_ratio"] > 0.85 and wick_total < 0.15:
                self._add_pattern(
                    "Bearish Marubozu", "bearish_continuation", "SELL",
                    c["index"], c["x"], 0.7,
    ...
    def _detect_tweezer_top(self, candles: list):
        ...
        for i in range(1, len(candles)):
            ...
            if (prev["is_bullish"] and not curr["is_bullish"] and
                abs(prev["high"] - curr["high"]) / max(prev["range"], 1) < 0.03):
                self._add_pattern(
                    "Tweezer Top", "bearish_reversal", "SELL",
                    curr["index"], curr["x"], 0.65,
    ...
    def _detect_tweezer_bottom(self, candles: list):
        ...
        for i in range(1, len(candles)):
            ...
            if (not prev["is_bullish"] and curr["is_bullish"] and
                abs(prev["low"] - curr["low"]) / max(prev["range"], 1) < 0.03):
                self._add_pattern(
                    "Tweezer Bottom", "bullish_reversal", "BUY",
                    curr["index"], curr["x"], 0.65,
