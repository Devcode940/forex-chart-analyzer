...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Vectorized using 2D NumPy array operations for speed (~500ms -> ~7ms).
        """
        np.random.seed(42)
        ...
        # 0: bullish (0.3), 1: bearish (0.3), 2: ranging (0.25), 3: volatile (0.15)
        trend_idx = np.random.choice(4, size=n_samples, p=[0.3, 0.3, 0.25, 0.15])
        mus = np.array([0.002, -0.002, 0.0, 0.0])[trend_idx]
        sigmas = np.array([0.008, 0.008, 0.005, 0.02])[trend_idx]

        # Target labels based on trend type
        y[trend_idx == 0] = 1
        y[trend_idx == 1] = 0
        n_ranging = np.sum(trend_idx == 2)
        if n_ranging > 0:
            y[trend_idx == 2] = np.random.choice([0, 1], size=n_ranging)
        n_volatile = np.sum(trend_idx == 3)
        if n_volatile > 0:
            y[trend_idx == 3] = np.random.choice([0, 1], size=n_volatile, p=[0.55, 0.45])
        ...
        center = np.where(trend_idx == 0, 0.01, -0.01)
    ...
    def _train_ensemble(self, X, y):
        """Train the stacked ensemble with tuned estimators."""
        # Scale features
        X_scaled = self.scaler.fit_transform(X)

        # Base learners (tuned estimator counts for optimal performance/accuracy trade-off)
    ...
    def _cross_validate(self, X, y) -> dict:
        ...
        try:
            # Parallelize cross-validation across CPU cores while avoiding thread oversubscription
