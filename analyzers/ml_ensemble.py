...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Vectorized with NumPy 2D operations for high performance.
        ...
        # Trend types: 0: bullish, 1: bearish, 2: ranging, 3: volatile
        ...
        mask_bull = (trend_types == 0)
        mask_bear = (trend_types == 1)
        mask_range = (trend_types == 2)
        mask_vol = (trend_types == 3)

        mu[mask_bull], sigma[mask_bull], y[mask_bull] = 0.002, 0.008, 1
        mu[mask_bear], sigma[mask_bear], y[mask_bear] = -0.002, 0.008, 0
        mu[mask_range], sigma[mask_range] = 0, 0.005
        if mask_range.any():
            y[mask_range] = np.random.choice([0, 1], size=mask_range.sum())
        mu[mask_vol], sigma[mask_vol] = 0, 0.02
        if mask_vol.any():
            y[mask_vol] = np.random.choice([0, 1], size=mask_vol.sum(), p=[0.55, 0.45])

        # Generate returns matrix (n_samples, 20)
        ...
        X[:, 0] = ret[:, -1]
        X[:, 1] = ret[:, -5:].sum(axis=1)
        X[:, 2] = ret[:, -10:].sum(axis=1)
        X[:, 3] = ret.sum(axis=1)
        X[:, 4] = ret[:, -5:].sum(axis=1)
        X[:, 5] = ret[:, -10:].sum(axis=1)
        X[:, 6] = ret.sum(axis=1)
        X[:, 7] = ret[:, -5:].sum(axis=1) * 100
        X[:, 8] = ret[:, -10:].sum(axis=1) * 100
        X[:, 9] = ret.sum(axis=1) * 100
        ...
        X[:, 23] = mu * 100 + np.random.normal(0, 0.01, size=n_samples)
        X[:, 24] = mu * 80 + np.random.normal(0, 0.01, size=n_samples)
        X[:, 25] = mu * 50 + np.random.normal(0, 0.01, size=n_samples)
        X[:, 26] = np.random.normal(np.where(mask_bull, 0.01, -0.01), 0.02)
        X[:, 27] = np.random.normal(np.where(mask_bull, 0.01, -0.01), 0.02)
        X[:, 28] = np.random.normal(np.where(mask_bull, 0.01, -0.01), 0.02)
        X[:, 29] = np.abs(mu) / (sigma * 2 + 1e-8)
        ...
        X[:, 34] = mu * 50 + np.random.normal(0, 0.5, size=n_samples)
        X[:, 35] = mu * 50 + np.random.normal(0, 0.5, size=n_samples)
        X[:, 36] = np.random.uniform(0.5, 5, size=n_samples)
        X[:, 37] = mu * 10 + np.random.normal(0, 0.1, size=n_samples)
        ...
        X[:, 42] = mu / (sigma + 1e-8) * np.sqrt(252)
        X[:, 43] = mu / (sigma + 1e-8) * np.sqrt(252)
    ...
    def _augment_with_heuristics(self, feature_vector, patterns, structure,
                                  regime, confluence):
        """Create augmented samples from heuristic signal strengths using vectorized NumPy matrix ops."""
        n_aug = 200
    ...
    def _train_ensemble(self, X, y):
        """Train the stacked ensemble with tuned hyperparameters."""
        # Scale features
        X_scaled = self.scaler.fit_transform(X)

        # Base learners (optimized estimators and parallelized RF)
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run cross-validation on the base models using parallel execution."""
        X_scaled = self.scaler.transform(X)
