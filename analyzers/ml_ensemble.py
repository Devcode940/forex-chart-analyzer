...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Vectorized using 2D NumPy array operations for fast generation (~8ms vs ~500ms).
        ...
        r2_base = np.clip(np.abs(mus) / (sigmas + 1e-8) * 0.3 + np.random.normal(0, 0.1, size=n_samples), 0, 1)
        X[:, 20] = r2_base
