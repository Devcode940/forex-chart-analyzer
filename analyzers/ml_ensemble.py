...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Vectorized with 2D NumPy operations for maximum generation speed.
        """
        np.random.seed(42)
        n_features = 50  # Match FeatureEngineer output

        trend_type = np.random.choice([0, 1, 2, 3], size=n_samples, p=[0.3, 0.3, 0.25, 0.15])
        mu = np.zeros(n_samples)
        sigma = np.zeros(n_samples)
        y = np.zeros(n_samples, dtype=int)

        bull_mask = (trend_type == 0)
        bear_mask = (trend_type == 1)
        range_mask = (trend_type == 2)
        vol_mask = (trend_type == 3)

        mu[bull_mask] = 0.002
        sigma[bull_mask] = 0.008
        y[bull_mask] = 1

        mu[bear_mask] = -0.002
        sigma[bear_mask] = 0.008
        y[bear_mask] = 0

        mu[range_mask] = 0.0
        sigma[range_mask] = 0.005
        ...
        # Generate 20 returns per sample in a single matrix draw
        ...
        sma_offset = np.where(bull_mask, 0.01, -0.01)
        X[:, 26] = np.random.normal(sma_offset, 0.02)
        X[:, 27] = np.random.normal(sma_offset, 0.02)
        X[:, 28] = np.random.normal(sma_offset, 0.02)
    ...
    def _augment_with_heuristics(self, feature_vector, patterns, structure,
        ...
        if bull_score > bear_score + 0.1:
            ...
            y_aug = (np.random.random(n_aug) < 0.3).astype(int)
        else:
    ...
    def _train_ensemble(self, X, y):
        """Train the stacked ensemble with optimized estimator parameters."""
        X_scaled = self.scaler.fit_transform(X)

        # Base learners tuned for fast training and low thread contention
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run cross-validation on the base models using parallel folds."""
        X_scaled = self.scaler.transform(X)

        try:
            # Set n_jobs=1 on internal RF estimator to avoid nested thread oversubscription during cross_val_score(..., n_jobs=-1)
