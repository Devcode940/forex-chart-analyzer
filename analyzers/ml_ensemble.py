...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Fully vectorized with 2D NumPy array operations for speed (~20x faster).
        """
        np.random.seed(42)
        n_features = 50  # Match FeatureEngineer output

        # Draw trend types for all samples at once: 0=bullish, 1=bearish, 2=ranging, 3=volatile
        trend_types = np.random.choice([0, 1, 2, 3], size=n_samples, p=[0.3, 0.3, 0.25, 0.15])

        y = np.zeros(n_samples, dtype=int)
        y[trend_types == 0] = 1
        y[trend_types == 1] = 0
        n_ranging = np.sum(trend_types == 2)
        if n_ranging > 0:
            y[trend_types == 2] = np.random.choice([0, 1], size=n_ranging)
        n_volatile = np.sum(trend_types == 3)
        if n_volatile > 0:
            y[trend_types == 3] = np.random.choice([0, 1], size=n_volatile, p=[0.55, 0.45])
        ...
        # 2D returns array: shape (n_samples, 20)
        ...
        X[:, 14] = sigmas * 10
        X[:, 15] = X[:, 10] / (X[:, 12] + 1e-8)
        X[:, 16] = np.random.uniform(0.1, 0.5, n_samples)
        X[:, 17] = np.random.uniform(0.1, 0.5, n_samples)
        X[:, 18] = np.random.uniform(0.3, 0.9, n_samples)
        X[:, 19] = np.random.uniform(0.001, 0.01, n_samples)

        # Trend features (20-29)
        r2 = np.clip(np.abs(mus) / (sigmas + 1e-8) * 0.3 + np.random.normal(0, 0.1, n_samples), 0, 1)
        X[:, 20] = r2
        X[:, 21] = mus / (sigmas + 1e-8)
        X[:, 22] = np.random.uniform(-0.1, 0.1, n_samples)
        X[:, 23] = mus * 100 + np.random.normal(0, 0.01, n_samples)
        X[:, 24] = mus * 80 + np.random.normal(0, 0.01, n_samples)
        X[:, 25] = mus * 50 + np.random.normal(0, 0.01, n_samples)

        bull_mask = (trend_types == 0)
        X[:, 26] = np.random.normal(np.where(bull_mask, 0.01, -0.01), 0.02)
        X[:, 27] = np.random.normal(np.where(bull_mask, 0.01, -0.01), 0.02)
        X[:, 28] = np.random.normal(np.where(bull_mask, 0.01, -0.01), 0.02)
        X[:, 29] = np.abs(mus) / (sigmas * 2 + 1e-8)

        # Structure features (30-39)
        X[:, 30] = np.random.randint(1, 6, n_samples)
        X[:, 31] = np.random.randint(1, 6, n_samples)
        X[:, 32] = np.random.randint(1, 10, n_samples)
        X[:, 33] = np.random.randint(1, 10, n_samples)
        X[:, 34] = mus * 50 + np.random.normal(0, 0.5, n_samples)
        X[:, 35] = mus * 50 + np.random.normal(0, 0.5, n_samples)
        X[:, 36] = np.random.uniform(0.5, 5, n_samples)
        X[:, 37] = mus * 10 + np.random.normal(0, 0.1, n_samples)
        X[:, 38] = np.random.randint(0, 3, n_samples)
        X[:, 39] = np.random.randint(0, 3, n_samples)

        # Statistical features (40-49)
        X[:, 40] = np.random.normal(0, 0.5, n_samples)
        X[:, 41] = np.random.normal(0, 1, n_samples)
        X[:, 42] = mus / (sigmas + 1e-8) * np.sqrt(252)
        X[:, 43] = mus / (sigmas + 1e-8) * np.sqrt(252)
        X[:, 44] = np.random.uniform(-0.15, -0.01, n_samples)
        X[:, 45] = -sigmas * 1.65
        X[:, 46] = -sigmas * 2.0
        X[:, 47] = np.random.uniform(0.3, 0.7, n_samples)
        X[:, 48] = np.random.uniform(0.2, 0.8, n_samples)
        X[:, 49] = np.random.uniform(-0.2, 0.2, n_samples)
    ...
    def _augment_with_heuristics(self, feature_vector, patterns, structure,
        ...
        y_aug = (np.random.random(n_aug) < p_bull).astype(int)
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners (optimized estimator counts for execution efficiency)
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run cross-validation on the base models in parallel."""
        X_scaled = self.scaler.transform(X)
