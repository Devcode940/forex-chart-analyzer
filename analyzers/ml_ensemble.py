...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Optimized: Fully vectorized with NumPy 2D array operations for fast generation.
        """
        np.random.seed(42)
        ...
        # Vectorized trend selection: 0=bullish, 1=bearish, 2=ranging, 3=volatile
        trend_types = np.random.choice([0, 1, 2, 3], size=n_samples, p=[0.3, 0.3, 0.25, 0.15])

        # Vectorized labels based on trend type
        y = np.where(
            trend_types == 0, 1,
            np.where(
                trend_types == 1, 0,
                np.where(
                    trend_types == 2, np.random.randint(0, 2, size=n_samples),
                    np.random.choice([0, 1], size=n_samples, p=[0.55, 0.45])
                )
            )
        )
        ...
        X[:, 6] = X[:, 3]
        base_price = 1.0
        X[:, 7] = (X[:, 1] / base_price * 100)
        X[:, 8] = (X[:, 2] / base_price * 100)
        X[:, 9] = (X[:, 3] / base_price * 100)
        ...
        r2 = np.maximum(0, np.minimum(1, np.abs(mus) / (sigmas + 1e-8) * 0.3 + np.random.normal(0, 0.1, size=n_samples)))
        X[:, 20] = r2
        ...
        trend_offset = np.where(trend_types == 0, 0.01, -0.01)
        X[:, 26] = np.random.normal(trend_offset, 0.02)
        X[:, 27] = np.random.normal(trend_offset, 0.02)
        X[:, 28] = np.random.normal(trend_offset, 0.02)
    ...
    def _augment_with_heuristics(self, feature_vector, patterns, structure,
        ...
        # Determine label probability from confluence heuristics
        ...
        y_aug = np.random.binomial(1, p, size=n_aug)
        return X_aug, y_aug
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners (tuned hyperparameters for optimal speed/accuracy trade-off)
