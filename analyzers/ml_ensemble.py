...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Fully vectorized with 2D NumPy operations (~130x faster than Python loop).
        ...
        trends = np.random.choice([0, 1, 2, 3], size=n_samples, p=[0.3, 0.3, 0.25, 0.15])
        mus = np.array([0.002, -0.002, 0.0, 0.0])[trends]
        sigmas = np.array([0.008, 0.008, 0.005, 0.02])[trends]

        # Labels
        y = np.where(trends == 0, 1,
            np.where(trends == 1, 0,
            np.where(trends == 2, np.random.choice([0, 1], size=n_samples),
                     np.random.choice([0, 1], size=n_samples, p=[0.55, 0.45]))))

        # Vectorized returns generation: (n_samples, 20)
        ...
        X[:, 10] = ret[:, -5:].std(axis=1)
        X[:, 11] = ret[:, -10:].std(axis=1)
        X[:, 12] = ret.std(axis=1)
        X[:, 13] = sigmas * 1.5
        X[:, 14] = sigmas * 10.0
        X[:, 15] = X[:, 10] / (X[:, 12] + 1e-8)
        ...
        bull_mask = (trends == 0)
        center = np.where(bull_mask, 0.01, -0.01)
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners (tuned estimators & n_jobs=1 to prevent thread contention)
    ...
    def _cross_validate(self, X, y) -> dict:
        ...
        try:
            # Pass n_jobs=-1 on cross_val_score and n_jobs=1 on underlying estimators to maximize parallelism without thread oversubscription
