...
class MLEnsemble:
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners (tuned estimator count for faster execution)
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run cross-validation on base learners (cv=3, single-threaded estimator to avoid process oversubscription)."""
        X_scaled = self.scaler.transform(X)
