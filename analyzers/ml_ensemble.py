...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Optimized with vectorized 2D NumPy operations (~500ms down to ~8ms).
        """
        np.random.seed(42)
        n_features = 50  # Match FeatureEngineer output

        # Vectorized trend type selection
        types = np.random.choice([0, 1, 2, 3], size=n_samples, p=[0.3, 0.3, 0.25, 0.15])
        mus = np.select([types == 0, types == 1, types == 2, types == 3], [0.002, -0.002, 0.0, 0.0])
        sigmas = np.select([types == 0, types == 1, types == 2, types == 3], [0.008, 0.008, 0.005, 0.02])

        y = np.zeros(n_samples, dtype=int)
        y[types == 0] = 1
        y[types == 1] = 0
        y[types == 2] = np.random.choice([0, 1], size=np.sum(types == 2))
        y[types == 3] = np.random.choice([0, 1], size=np.sum(types == 3), p=[0.55, 0.45])
        ...
        bull_mask = (types == 0)
        sma_loc = np.where(bull_mask, 0.01, -0.01)
        X[:, 26] = np.random.normal(sma_loc, 0.02)
        X[:, 27] = np.random.normal(sma_loc, 0.02)
        X[:, 28] = np.random.normal(sma_loc, 0.02)
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners (tuned estimators for faster training)
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run cross-validation on the base models (parallelized CV with n_jobs=-1)."""
        X_scaled = self.scaler.transform(X)
