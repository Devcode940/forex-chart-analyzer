...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        Vectorized using 2D NumPy arrays for ~80x faster generation.
        """
        np.random.seed(42)
        ...
        mu[range_mask], sigma[range_mask] = 0, 0.005
        y[range_mask] = np.random.choice([0, 1], size=np.sum(range_mask))

        mu[vol_mask], sigma[vol_mask] = 0, 0.02
        y[vol_mask] = np.random.choice([0, 1], size=np.sum(vol_mask), p=[0.55, 0.45])

        # Generate base returns distribution (shape: n_samples x 20)
        ret = np.random.normal(mu[:, None], sigma[:, None], (n_samples, 20))
        ...
        X[:, 10] = np.std(ret[:, -5:], axis=1, ddof=0)
        X[:, 11] = np.std(ret[:, -10:], axis=1, ddof=0)
        X[:, 12] = np.std(ret, axis=1, ddof=0)
        X[:, 13] = sigma * 1.5
        X[:, 14] = sigma * 10
        X[:, 15] = X[:, 10] / (X[:, 12] + 1e-8)
        ...
        r2_raw = np.abs(mu) / (sigma + 1e-8) * 0.3 + np.random.normal(0, 0.1, size=n_samples)
        ...
        sma_means = np.where(bull_mask, 0.01, np.where(bear_mask, -0.01, -0.01))
        X[:, 26] = np.random.normal(sma_means, 0.02)
        X[:, 27] = np.random.normal(sma_means, 0.02)
        X[:, 28] = np.random.normal(sma_means, 0.02)
    ...
    def _augment_with_heuristics(self, feature_vector, patterns, structure,
                                  regime, confluence):
        """Vectorized augmentation of feature vectors using heuristic signals."""
        n_aug = 200
        ...
        if bull_score > bear_score + 0.1:
            y_aug = np.random.choice([1, 0], size=n_aug, p=[0.7, 0.3])
        elif bear_score > bull_score + 0.1:
            y_aug = np.random.choice([0, 1], size=n_aug, p=[0.7, 0.3])
        else:
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners - tuned n_estimators for balance of speed & performance
    ...
    def _cross_validate(self, X, y) -> dict:
        """Run parallel cross-validation on the base models."""
        X_scaled = self.scaler.transform(X)

        try:
            # Set n_jobs=1 on estimator to prevent thread oversubscription when cross_val_score uses n_jobs=-1
