...
class MLEnsemble:
    ...
    _cache = (
        OrderedDict()
    )  # Bounded LRU cache to avoid redundant training on unchanged inputs
    ...
    def _make_cache_key(
        ...
        """Create MD5 hash cache key from inputs."""
        try:
            fv_bytes = (
                feature_vector.tobytes()
                if isinstance(feature_vector, np.ndarray)
                else str(feature_vector).encode()
            )
            p_str = json.dumps(pattern_results, sort_keys=True, default=str)
            s_str = json.dumps(structure_results, sort_keys=True, default=str)
            r_str = json.dumps(regime_results, sort_keys=True, default=str)
            c_str = json.dumps(confluence_results, sort_keys=True, default=str)
            return hashlib.md5(
                fv_bytes
                + p_str.encode()
                + s_str.encode()
    ...
    def train_and_predict(
        ...
        Uses bounded LRU caching to optimize performance across UI re-executions.
        """
        ...
        if cache_key and cache_key in self._cache:
            ...
            # Restore internal fitted state attributes
            self.rf_model = cached_entry["rf_model"]
            self.gb_model = cached_entry["gb_model"]
            self.meta_model = cached_entry["meta_model"]
            self.scaler = cached_entry["scaler"]
            self.is_trained = cached_entry["is_trained"]
            self.training_stats = cached_entry["training_stats"]
    ...
    def _train_ensemble(self, X, y):
        ...
        # Base learners with balanced hyperparameters for speed and accuracy
    ...
    def _cross_validate(self, X, y) -> dict:
        ...
        try:
            # Parallel cross-validation with cv=3 and balanced estimators
