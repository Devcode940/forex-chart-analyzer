...
class MLEnsemble:
    ...
    def _generate_synthetic_data(self, n_samples: int = 2000):
        ...
        y[trend_types == "ranging"] = np.random.choice(
            [0, 1], size=np.sum(trend_types == "ranging")
        )
        y[trend_types == "volatile"] = np.random.choice(
            [0, 1], size=np.sum(trend_types == "volatile"), p=[0.55, 0.45]
        )
