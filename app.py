...
def run_analysis(image: np.ndarray, account_balance: float, risk_pct: float, pair: str):
    ...
    with st.spinner("📐 Analyzing market structure..."):
        structure = (
            st.session_state.structure_analyzer.analyze(all_points, image.shape[0])
            if all_points
            else {}
        )
    ...
    with st.spinner("🔍 Detecting geometric patterns..."):
        patterns = (
            st.session_state.pattern_detector.detect_all(
    ...
    with st.spinner("🕯️ Detecting candlestick patterns..."):
        candlesticks = (
            st.session_state.candlestick_detector.detect_all(
    ...
    with st.spinner("📊 Identifying support & resistance..."):
        sr = (
            st.session_state.sr_detector.detect(
                structure.get("price_series", {}), image.shape[0]
            )
            if structure.get("price_series")
    ...
    with st.spinner("🌐 Classifying market regime..."):
        regime = (
            st.session_state.regime_classifier.classify(
    ...
    with st.spinner("📈 Calculating Fibonacci levels..."):
        fib = (
            st.session_state.fibonacci_calculator.calculate(structure, None)
            if structure
            else {}
        )
    ...
    with st.spinner("📉 Detecting divergences..."):
        divergences = (
            st.session_state.divergence_detector.detect_all(
                structure.get("price_series", {}), structure
            )
            if structure.get("price_series")
            else []
    ...
    with st.spinner("🎯 Detecting indicator lines..."):
        ...
        st.session_state.ma_crossovers = (
            st.session_state.indicator_detector.get_ma_crossovers(indicators)
        )

    with st.spinner("💧 Detecting liquidity zones..."):
        liquidity = (
            st.session_state.liquidity_detector.detect_all(
    ...
    with st.spinner("⏰ Analyzing session context..."):
        session = (
            st.session_state.session_analyzer.analyze(
    ...
    with st.spinner("💰 Calculating SL/TP levels..."):
        sltp = (
            st.session_state.sltp_calculator.calculate(
                patterns, sr, structure, structure.get("price_series", {})
            )
            if structure.get("price_series")
            else {"scenarios": [], "bias": "NEUTRAL"}
        )
    ...
    with st.spinner("⚖️ Calculating risk management..."):
        risk = st.session_state.risk_manager.calculate(
            sltp, sr, structure, image.shape[0], account_balance, risk_pct, pair
        )
...
# ─── MAIN UI ───────────────────────────────────────────────────────────────────
st.markdown(
    '<h1 class="main-header">🔍 Forex Chart Analyzer</h1>', unsafe_allow_html=True
)
st.markdown(
    '<p class="sub-header">Pattern Recognition • Candlestick Analysis • Fibonacci • Divergence • Liquidity Zones • Confluence Scoring • Risk Management</p>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    account_balance = st.number_input(
        "Account Balance ($)", value=10000.0, min_value=100.0, step=500.0
    )
    ...
    if st.session_state.analysis_complete:
        ...
        with col_conf2:
            ...
            bias_color = (
                "#00ff88"
                if bias == "BULLISH"
                else "#ff4444" if bias == "BEARISH" else "#ffdd44"
            )
            st.markdown(
                f"""
            <div class="metric-card">
        ...
        with tabs[0]:
            ...
            filtered = [
                p for p in patterns if p.get("confidence", 0) >= confidence_threshold
            ]
            if filtered:
                ...
                for p in filtered:
                    ...
                    css = (
                        "bullish"
                        if "BULLISH" in pt
                        else "bearish" if "BEARISH" in pt else "neutral"
                    )
                ...
                for pattern in filtered:
                    icon = (
                        ...
                        else "🔴" if "BEARISH" in pattern.get("type", "") else "🟡"
                    )
                    with st.expander(
                        f"{icon} {pattern['name']} — {pattern['confidence']:.0%}"
                    ):
                        st.markdown(f"**Type:** {pattern.get('type', 'N/A')}")
                        st.markdown(
                            f"**Direction:** {pattern.get('target_direction', 'PENDING')}"
                        )
                        st.markdown(
        ...
        with tabs[1]:
            ...
            if candlesticks:
                ...
                neutral_cs = [
                    ...
                    if c.get("signal") in ["REVERSAL_POSSIBLE", "WAIT"]
                ]
                ...
                for c in candlesticks:
                    icon = (
                        "🟢"
                        if c.get("signal") == "BUY"
                        else "🔴" if c.get("signal") == "SELL" else "🟡"
                    )
                    with st.expander(
                        f"{icon} {c['name']} ({c.get('category', '')}) — {c['confidence']:.0%}"
                    ):
        ...
        with tabs[2]:
            ...
            with col_st1:
                ...
                for bos in structure.get("structure_breaks", []):
                    ...
                    st.markdown(
                        f"  {icon} **{bos['type']}** at index {bos.get('index', '?')}"
                    )

                st.markdown("#### Phases")
                for phase in structure.get("phases", []):
                    st.markdown(
                        f"- **{phase['phase']}** (bars {phase['start']}-{phase['end']}, slope: {phase['slope']:.3f})"
                    )
                    ...
                    for sh in structure.get("swing_highs", []):
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="markers",
                                marker=dict(
                                    color="#ff4444", size=8, symbol="triangle-down"
                                ),
                                showlegend=False,
                            )
                        )
                    for sl in structure.get("swing_lows", []):
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="markers",
                                marker=dict(
                                    color="#00ff88", size=8, symbol="triangle-up"
                                ),
                                showlegend=False,
                            )
                        )
                    fig.update_layout(
                        title="Price Structure", height=350, template="plotly_dark"
                    )
        ...
        with tabs[3]:
            ...
            with col_fib1:
                ...
                for i, s in enumerate(sr.get("support", [])):
                    st.markdown(
                        f'<div class="fib-zone" style="border-color:#00ff88;"><span style="color:#00ff88;">🟢 Support #{i+1}</span> — Strength: {s.get("strength",0):.0%} — Touches: {s.get("touches",0)}</div>',
                        unsafe_allow_html=True,
                    )
                for i, r in enumerate(sr.get("resistance", [])):
                    st.markdown(
                        f'<div class="fib-zone" style="border-color:#ff4444;"><span style="color:#ff4444;">🔴 Resistance #{i+1}</span> — Strength: {r.get("strength",0):.0%} — Touches: {r.get("touches",0)}</div>',
                        unsafe_allow_html=True,
                    )
                ...
                if sr.get("key_zones"):
                    ...
                    for z in sr["key_zones"]:
                        st.markdown(
                            f'<div class="fib-zone" style="border-color:#ff8800;"><span style="color:#ff8800;">⚡ Confluence</span> at {z.get("level",0):.1f}</div>',
                            unsafe_allow_html=True,
                        )
            ...
            with col_fib2:
                ...
                st.markdown(
                    f"*Swing: {fib.get('swing_low',{}).get('value',0):.1f} → {fib.get('swing_high',{}).get('value',0):.1f}*"
                )
                ...
                for ratio in [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]:
                    if ratio in retracements:
                        ...
                        color = (
                            "#ff8800"
                            if level.get("importance") == "HIGH"
                            else "#888888"
                        )
                        st.markdown(
                            f'<div class="fib-zone" style="border-color:{color};"><span style="color:{color};">{level["label"]}</span> → <strong>{level["value"]:.2f}</strong> ({level.get("direction","")})</div>',
                            unsafe_allow_html=True,
                        )
                ...
                if trade_zones:
                    ...
                    for tz in trade_zones:
                        st.markdown(
                            f"- **{tz['zone_name']}** @ {tz['price']:.2f} — {tz['action']} ({tz['importance']})"
                        )
                ...
                if len(smoothed) > 14:
                    ...
                    fig_mom.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            name="Momentum",
                            line=dict(color="#ff88ff"),
                        )
                    )
                    fig_mom.update_layout(
                        title="Price vs Momentum (Divergence Check)",
                        height=250,
        ...
        with tabs[5]:
            ...
            with col_l4:
                st.metric(
                    "Pending Hunt Zones", liq_summary.get("pending_hunt_zones", 0)
                )

            # Summary interpretation
            if liq_summary.get("interpretation"):
                st.info(
                    f"🧠 **Liquidity Intelligence:** {liq_summary['interpretation']}"
                )
            ...
            with col_liq1:
                ...
                for bs in liquidity.get("buy_side_liquidity", []):
                    st.markdown(
                        f'<div class="liquidity-zone" style="background:#3a1a1a;border:1px solid #ff444466;"><strong style="color:#ff4444;">⬆ {bs["source"]}</strong> at {bs["level"]:.1f}<br><span style="color:#8899aa;font-size:0.8rem;">{bs["description"]}</span></div>',
                        unsafe_allow_html=True,
                    )
                ...
                for ss in liquidity.get("sell_side_liquidity", []):
                    st.markdown(
                        f'<div class="liquidity-zone" style="background:#1a3a2a;border:1px solid #00ff8866;"><strong style="color:#00ff88;">⬇ {ss["source"]}</strong> at {ss["level"]:.1f}<br><span style="color:#8899aa;font-size:0.8rem;">{ss["description"]}</span></div>',
                        unsafe_allow_html=True,
                    )
            ...
            with col_liq2:
                ...
                if sweeps:
                    ...
                    for sweep in sweeps:
                        color = (
                            "#ff4444" if "BUY" in sweep.get("type", "") else "#00ff88"
                        )
                        st.markdown(
                            f'<div class="liquidity-zone" style="background:{"#3a1a1a" if "BUY" in sweep.get("type","") else "#1a3a2a"};border:2px solid {color};"><strong style="color:{color};">⚡ {sweep["type"].replace("_"," ")}</strong> at {sweep["level"]:.1f}<br>{sweep["description"]}<br><strong>Action:</strong> {sweep["action"]}</div>',
                            unsafe_allow_html=True,
                        )
                else:
                    st.markdown("#### ⚡ No Recent Sweeps")
                    st.info(
                        "No liquidity sweeps detected recently. Pending zones are targets for smart money."
                    )
                ...
                if stop_hunt_zones:
                    ...
                    for zone in stop_hunt_zones:
                        st.warning(
                            f"**Target:** {zone['level']:.1f} — {zone['description']}"
                        )
                ...
                if equal_levels:
                    ...
                    for eq in equal_levels:
                        st.markdown(
                            f"**{eq['type'].replace('_',' ').title()}:** {eq['level']:.1f} — {eq['description']}"
                        )
        ...
        with tabs[6]:
            ...
            if best_risk:
                ...
                with col_r1:
                    st.metric(
                        "Recommended Lots",
                        f"{best_risk.get('recommended_lots', 0):.2f}",
                    )
                with col_r2:
                    st.metric(
                        "Risk Amount", f"${best_risk.get('risk_amount_usd', 0):.2f}"
                    )
                with col_r3:
                    st.metric(
                        "Profit Potential",
                        f"${best_risk.get('profit_potential_usd', 0):.2f}",
                    )
                with col_r4:
                    st.metric(
                        "SL Distance",
                        f"{best_risk.get('sl_distance_pips', 0):.1f} pips",
                    )
            ...
            if ror:
                ...
                ror_color = {
                    ...
                    "HIGH": "#ff8844",
                    "VERY_HIGH": "#ff4444",
                }.get(ror_level, "#ffffff")
                st.markdown(
                    f"**Level:** <span style='color:{ror_color};font-weight:700;'>{ror_level}</span> ({ror.get('ror', 0):.2%})",
                    unsafe_allow_html=True,
                )
                st.markdown(
                    f"**Kelly %:** {ror.get('kelly_pct', 0):.1f}% — Recommended max risk: {ror.get('recommended_max_risk', 0):.2f}%"
                )
                if ror.get("advice"):
                    st.info(ror["advice"])
            ...
            if max_dd:
                ...
                st.markdown(
                    f"- **Expected:** {max_dd.get('expected_max_dd_pct', 0):.1f}%"
                )
                st.markdown(
                    f"- **Worst Case:** {max_dd.get('worst_case_dd_pct', 0):.1f}%"
                )
                st.markdown(
                    f"- **Recovery Needed:** {max_dd.get('recovery_needed', 0):.1f}%"
                )
        ...
        with tabs[7]:
            ...
            with col_reg2:
                ...
                if best_session:
                    st.info(
                        f"🏆 **Best session for {pair}:** {best_session.get('best', 'N/A')}"
                    )
        ...
        with tabs[8]:
                        ...
                        gauge={
                            ...
                            "bar": {
                                "color": (
                                    "#00ff88"
                                    if confidence > 0.65
                                    else "#ffdd44" if confidence > 0.45 else "#ff4444"
                                )
                            },
                            "steps": [
            ...
            for sig in confluence.get("signals", []):
                dir_color = (
                    ...
                    else "#ff4444" if sig["direction"] == "BEARISH" else "#ffdd44"
                )
                dir_icon = (
                    "🟢"
                    if sig["direction"] == "BULLISH"
                    else "🔴" if sig["direction"] == "BEARISH" else "🟡"
                )
                st.markdown(
                    f"{dir_icon} **{sig['source']}:** {sig['name']} — Strength: {sig['strength']:.0%} (weight: {sig['weight']:.1f}x)"
                )
            ...
            if trade_plan.get("action") != "DO NOT TRADE":
                ...
                st.warning(
                    f"⚠️ {trade_plan.get('reason', 'Conflicting signals — wait for clarity.')}"
                )
        ...
        with tabs[9]:
            ...
            if st.button(
                ...
                if not ps:
                    ...
                    with st.spinner(
                        "🎰 Running Monte Carlo Simulation (2000 paths)..."
                    ):
                        mc = validator.monte_carlo_validation(
                            ps, patterns, structure, n_simulations=2000
                        )
                        st.session_state.mc_results = mc

                    with st.spinner(
                        "🔄 Running Bootstrap Resampling (5000 samples)..."
                    ):
                        bs = validator.bootstrap_confidence_interval(
                            ps, n_bootstrap=5000
                        )
            ...
            if "audit_results" in st.session_state:
                ...
                for vote in votes:
                    v_color = (
                        "#00ff88"
                        if vote["verdict"] == "PASS_85"
                        else (
                            "#ffdd44"
                            if vote["verdict"] in ["SIGNIFICANT", "MODERATE"]
                            else "#ff4444"
                        )
                    )
                    v_icon = (
                    ...
                    st.markdown(
                        f'{v_icon} **{vote["method"]}**: {vote["verdict"]} — Probability: {vote["prob"]:.1%}'
                    )
                ...
                with col_sv1:
                    ...
                    if mc and "error" not in mc:
                        with st.expander("🎰 Monte Carlo Simulation", expanded=True):
                            st.markdown(
                                f"**Heuristic Confidence:** {mc.get('pattern_heuristic_confidence', 0):.1%}"
                            )
                            st.markdown(
                                f"**Random Baseline:** {mc.get('random_baseline_win_rate', 0):.1%}"
                            )
                            st.markdown(
                                f"**Statistical Probability:** {mc.get('statistical_probability', 0):.1%}"
                            )
                            st.markdown(f"**P-value:** {mc.get('p_value', 1):.4f}")
                            st.markdown(
                                f"**Statistically Significant:** {'✅ Yes' if mc.get('is_statistically_significant') else '❌ No'}"
                            )
                            st.markdown(
                                f"**Exceeds 85%:** {'✅ Yes' if mc.get('exceeds_85_confidence') else '❌ No'}"
                            )
                    ...
                    if ent and "error" not in ent:
                        with st.expander("📐 Shannon Entropy Analysis", expanded=True):
                            st.markdown(
                                f"**Entropy:** {ent.get('entropy_bits', 0):.4f} bits (max 1.0)"
                            )
                            st.markdown(
                                f"**Predictability:** {ent.get('predictability', 0):.1%}"
                            )
                            st.markdown(
                                f"**Mutual Information:** {ent.get('mutual_information', 0):.4f} bits"
                            )
                            st.markdown(
                                f"**P(up):** {ent.get('p_up', 0):.1%} | P(down):** {ent.get('p_down', 0):.1%}"
                            )
                ...
                with col_sv2:
                    ...
                    if bs and "error" not in bs:
                        with st.expander(
                            "🔄 Bootstrap Confidence Intervals", expanded=True
                        ):
                            pe = bs.get("point_estimates", {})
                            st.markdown(
                                f"**Win Rate Estimate:** {pe.get('win_rate', 0):.1%}"
                            )
                            st.markdown(
                                f"**Trend Strength:** {pe.get('trend_strength', 0):.3f}"
                            )
                            ...
                            for level, data in ci.items():
                                ...
                                st.markdown(
                                    f"**{level.replace('ci_', '')}% CI for Win Rate:** [{wr_ci[0]:.1%}, {wr_ci[1]:.1%}]"
                                )
                    ...
                    if mk and "error" not in mk:
                        with st.expander("🔗 Markov Chain Analysis", expanded=True):
                            st.markdown(
                                f"**Current State:** {mk.get('current_state', '?')}"
                            )
                            st.markdown(
                                f"**Persistence Probability:** {mk.get('persistence_probability', 0):.1%}"
                            )
                            st.markdown(
                                f"**Next Bar Bullish:** {mk.get('next_bar_bullish_prob', 0):.1%}"
                            )
                            st.markdown(
                                f"**Next Bar Bearish:** {mk.get('next_bar_bearish_prob', 0):.1%}"
                            )
                            st.markdown(
                                f"**5-Bar Bullish:** {mk.get('five_bar_bullish_prob', 0):.1%}"
                            )
                            st.markdown(
                                f"**Can Claim 85% (1-bar):** {'✅' if mk.get('can_claim_85_next_bar') else '❌'}"
                            )
                            st.markdown(
                                f"**Can Claim 85% (5-bar):** {'✅' if mk.get('can_claim_85_five_bar') else '❌'}"
                            )
        ...
        with tabs[10]:
            ...
            st.caption(
                "Ensemble ML | Isolation Forest | Walk-Forward Validation | Probability Calibration | Meta-Learner"
            )
            ...
            if st.button(
                ...
                if not ps:
                    ...
                    with st.spinner(
                        ...
                        ml_result = ml.train_and_predict(
                            feat_result["feature_vector"],
                            patterns,
                            structure,
                            regime,
                            confluence,
                    ...
                    with st.spinner(
                        "Detecting market anomalies (Isolation Forest + PCA)..."
                    ):
                    ...
                    with st.spinner("Calibrating probabilities (Platt + Isotonic)..."):
                        ...
                        cal_result = cal.calibrate_and_predict(
                            confluence,
                            patterns,
                            structure,
                            regime,
                    ...
                    if "audit_results" not in st.session_state:
                        with st.spinner("Running statistical validation..."):
                            ...
                            stat_result = sv.full_probability_audit(
                                ps,
                                patterns,
                                structure,
                                confluence,
                    ...
                    with st.spinner("Running Meta-Learner (combining all models)..."):
                        ...
                        meta_result = meta.predict_master(
                            feat_result["feature_vector"],
                            ml_result,
                            anomaly_result,
                            cal_result,
            ...
            if "meta_result" in st.session_state:
                ...
                st.markdown(
                    f"**ML MASTER PREDICTION:** Direction: **{master_dir}** | Grade: **{master_grade}** | Probability: **{master_prob:.1%}** | IC: **{ic:.3f}** | Agreement: **{agreement.replace('_',' ')}** | Position: **{risk_pos.get('position_strength','N/A')}** | Risk: **{risk_pos.get('recommended_risk_pct',0):.2f}%**"
                )
                ...
                for model_name, model_data in individual.items():
                    dir_color = (
                        "#00ff88"
                        if model_data["direction"] == "BULLISH"
                        else (
                            "#ff4444"
                            if model_data["direction"] == "BEARISH"
                            else "#ffdd44"
                        )
                    )
                    weight_pct = model_data["weight"] * 100
                    st.markdown(
                        f"**{model_name.replace('_',' ').title()}**: {model_data['direction']} ({model_data['prob']:.1%}) — Weight: {weight_pct:.0f}% — Source: {model_data['source']}"
                    )
                ...
                with ml_c1:
                    ...
                    if ml_r and "error" not in ml_r:
                        with st.expander("Ensemble Model Details"):
                            st.markdown(
                                f"RF Probability: {ml_r.get('rf_probability', 0):.1%}"
                            )
                            st.markdown(
                                f"GB Probability: {ml_r.get('gb_probability', 0):.1%}"
                            )
                            st.markdown(
                                f"Agreement: {'Yes' if ml_r.get('agreement') == 'YES' else 'No'}"
                            )
                            cv = ml_r.get("cv_score", {})
                            st.markdown(f"CV Score (RF): {cv.get('rf_cv_mean', 0):.1%}")
                            st.markdown(f"CV Score (GB): {cv.get('gb_cv_mean', 0):.1%}")
                            st.markdown(
                                f"Training Samples: {ml_r.get('training_samples', 0)}"
                            )
                            st.markdown("**Top Features:**")
                            for fi in ml_r.get("feature_importance", [])[:5]:
                                st.markdown(
                                    f"- {fi['feature']}: {fi['importance']:.3f}"
                                )
                    ...
                    if anom and "anomaly_level" in anom:
                        with st.expander("Anomaly Detection"):
                            ...
                            st.markdown(
                                f"Composite Score: {anom['anomaly_composite_score']:.3f}"
                            )
                ...
                with ml_c2:
                    ...
                    if wf_r and "error" not in wf_r:
                        with st.expander("Walk-Forward Validation"):
                            ...
                            for wr in wf_r.get("window_results", []):
                                st.markdown(
                                    f"Window {wr['window']}: Acc={wr['accuracy']:.1%}, PnL={wr['simulated_pnl']:.2f}, WR={wr['win_rate']:.1%}"
                                )
                    ...
                    if cal_r and "main_confluence" in cal_r:
                        with st.expander("Probability Calibration"):
                            ...
                            st.markdown(
                                f"Raw Heuristic: {mc.get('raw_heuristic', 0):.1%}"
                            )
                            st.markdown(
                                f"Platt Calibrated: {mc.get('platt_calibrated', 0):.1%}"
                            )
                            st.markdown(
                                f"Isotonic Calibrated: {mc.get('isotonic_calibrated', 0):.1%}"
                            )
                            st.markdown(
                                f"Best Calibrated: {mc.get('best_calibrated', 0):.1%}"
                            )
                            ...
                            st.markdown(
                                f"Calibration Quality: {cq.get('quality', 'N/A')}"
                            )
                            if mc.get("warning"):
                                st.warning(mc["warning"])
                ...
                with ml_c3:
                    ...
                    if feat_r and feat_r.get("n_features", 0) > 0:
                        with st.expander("Feature Engineering"):
                            ...
                            sorted_feats = sorted(
                                feats.items(), key=lambda x: abs(x[1]), reverse=True
                            )
        ...
        with tabs[11]:
            ...
            st.markdown(
                f"**Database:** {db_stats['total_trades']} trades ({db_stats['backtest_trades']} backtest, {db_stats['live_trades']} live) | {db_stats['patterns_tracked']} patterns | {db_stats['pattern_combos']} combos | Overall WR: **{db_stats['overall_win_rate']:.1%}** | PF: **{db_stats['profit_factor']:.2f}**"
            )

            if st.button(
                "🔍 Backtest Current Analysis Against Real Data",
                type="primary",
                use_container_width=True,
                key="run_backtest",
            ):
                ...
                bt_result = bt.backtest_current(
                    patterns, confluence_grade, regime_name, session_name, pair
                )
            ...
            if "bt_result" in st.session_state:
                ...
                # Pattern backtests
                st.markdown(
                    "#### 📊 Pattern-by-Pattern: Heuristic vs MEASURED Win Rate"
                )
                for pb in bt_result.get("pattern_backtests", []):
                    ...
                    over_color = (
                        "#ff4444"
                        if over > 0.10
                        else "#ffdd44" if over > 0.05 else "#00ff88"
                    )
                    st.markdown(
                        f"**{pb['pattern']}**: Heuristic **{pb['heuristic_confidence']:.0%}** → Measured **{pb['measured_win_rate']:.0%}** (n={pb['sample_size']}) | PF: {pb['measured_profit_factor']:.2f} | <span style='color:{over_color};'>Overconfidence: {over:+.0%}</span>",
                        unsafe_allow_html=True,
                    )
                ...
                if gb.get("sample_size", 0) > 0:
                    st.markdown(
                        f"#### 🎯 Confluence Grade: **{gb['grade']}** → Measured WR: **{gb['measured_win_rate']:.1%}** (n={gb['sample_size']})"
                    )
                ...
                if kp:
                    st.markdown(
                        f"#### 📐 Real Kelly Criterion (Setup: {bt_result.get('setup_type','N/A')})"
                    )
                    st.markdown(f"- **Measured Win Rate:** {kp['win_rate']:.1%}")
                    st.markdown(
                        f"- **Avg Win:** {kp['avg_win']:.1f} pips | **Avg Loss:** {kp['avg_loss']:.1f} pips"
                    )
                    st.markdown(f"- **Win/Loss Ratio:** {kp['win_loss_ratio']:.2f}")
                    st.markdown(
                        f"- **Full Kelly:** {kp['kelly_fraction']:.2%} | **Half Kelly:** {kp['half_kelly']:.2%}"
                    )
                    if kp["kelly_fraction"] > 0:
                        st.success(
                            f"✅ HAS EDGE: Half-Kelly recommends {kp['half_kelly']:.2%} risk per trade"
                        )
                    else:
                        st.error(
                            f"🔴 NO EDGE: Kelly is negative — this setup loses money historically"
                        )
                ...
                avg_heuristic = (
                    np.mean([p.get("confidence", 0.5) for p in patterns])
                    if patterns
                    else 0.5
                )
                cal_result = rc.calibrate(
                    avg_heuristic,
                    confluence.get("master", {}).get("grade", "D"),
                    patterns,
                )
                st.markdown("#### 🎯 Real Calibration (Database-Backed)")
                st.markdown(
                    f"**Raw Heuristic:** {cal_result['raw_heuristic']:.1%} → **Calibrated:** {cal_result['calibrated_probability']:.1%} (Adjustment: {cal_result['adjustment']:+.1%})"
                )
                if cal_result.get("is_overconfident"):
                    st.warning(
                        f"⚠️ OVERCONFIDENT by {abs(cal_result['adjustment']):.0%}. Your system claims more than history supports."
                    )
                elif cal_result.get("is_underconfident"):
                    st.success(
                        f"🟢 UNDERCONFIDENT — signal is actually stronger than scored!"
                    )
            ...
            for ps in all_stats:
                wr_color = (
                    "#00ff88"
                    if ps["win_rate"] > 0.60
                    else "#ffdd44" if ps["win_rate"] > 0.50 else "#ff4444"
                )
                st.markdown(
                    f"<span style='color:{wr_color};'>**{ps['name']}**</span>: {ps['win_rate']:.1%} WR | {ps['total_occurrences']} trades | PF: {ps['profit_factor']:.2f} | AvgW: {ps['avg_win_pips']:.0f}p AvgL: {ps['avg_loss_pips']:.0f}p",
                    unsafe_allow_html=True,
                )
            ...
            with st.form("trade_form"):
                ...
                t_pattern = st.text_input(
                    "Pattern Name", value=patterns[0]["name"] if patterns else ""
                )
                t_grade = st.text_input(
                    "Confluence Grade",
                    value=confluence.get("master", {}).get("grade", "C"),
                )
                t_regime = st.text_input(
                    "Regime", value=regime.get("regime", "TRENDING")
                )
                t_session = st.text_input(
                    "Session", value=session.get("inferred_session", "LONDON")
                )
                ...
                if submitted:
                    ...
                    st.success(
                        f"Trade #{trade_id} saved! Database now has {db.get_database_stats()['total_trades']} trades."
                    )
        ...
        with tabs[12]:
            ...
            st.caption(
                "The ONLY mathematically optimal position sizing method. Based on actual win rates, not estimates."
            )

            kelly_pair = st.selectbox(
                "Pair for Kelly",
                ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "XAUUSD"],
                key="kelly_pair",
            )
            sl_pips_input = st.number_input(
                "Stop Loss (pips)", value=50.0, min_value=5.0
            )
            setup_input = st.selectbox(
                "Setup Type",
                [
                    "confluence_a_grade",
                    "confluence_b_grade",
            ...
            if st.button(
                "📐 Calculate Real Kelly", type="primary", use_container_width=True
            ):
                rk = st.session_state.real_kelly
                kelly_result = rk.calculate(
                    setup_input, account_balance, sl_pips_input, kelly_pair
                )
            ...
            if "kelly_result" in st.session_state:
                ...
                if "error" in kr:
                    st.error(kr["error"])
                    st.info(
                        f"Fallback: Risk {kr.get('fallback_risk_pct', 0.5):.2%} → {kr.get('fallback_lots', 0):.2f} lots"
                    )
                else:
                    ...
                    st.markdown(
                        f"**Expected Value:** {kr['expected_value_pips']:+.1f} pips per trade"
                    )
                    st.markdown(f"**Risk of Ruin:** {kr['risk_of_ruin']:.2%}")
                    st.markdown(
                        f"**Trades to Double Account:** {kr['trades_to_double']}"
                    )
                    ...
                    st.markdown(f"|----------|--------|------|-------|")
                    st.markdown(
                        f"| Full Kelly | {kr['full_kelly']:.2%} | {st.session_state.real_kelly._calc_lots(account_balance, kr['full_kelly']/100 if kr['full_kelly']>0 else 0, sl_pips_input, kelly_pair):.2f} | ⚠️ Too aggressive |"
                    )
                    st.markdown(
                        f"| **Half Kelly** | **{kr['half_kelly']:.2%}** | **{kr['recommended_lots']:.2f}** | **Recommended** |"
                    )
                    st.markdown(
                        f"| Quarter Kelly | {kr['quarter_kelly']:.2%} | {st.session_state.real_kelly._calc_lots(account_balance, kr['quarter_kelly']/100 if kr['quarter_kelly']>0 else 0, sl_pips_input, kelly_pair):.2f} | Conservative |"
                    )
        ...
        with col_exp1:
            if st.button(
                "📥 Download Annotated Image", use_container_width=True, key="dl_img"
            ):
                if st.session_state.annotated_image is not None:
                    ...
                    st.download_button(
                        "💾 Save",
                        data=buf.getvalue(),
                        file_name="forex_analysis_v2.png",
                        mime="image/png",
                        use_container_width=True,
                    )
        ...
        with col_exp2:
            export = {
                "confluence": {
                    "grade": grade,
                    "direction": direction,
                    "bull_score": confluence.get("bull_score"),
                    "bear_score": confluence.get("bear_score"),
                },
                "regime": regime,
                "session": session,
                "structure": {
                    k: v for k, v in structure.items() if k != "price_series"
                },
                "geometric_patterns": [
                    {
                        "name": p["name"],
                ...
                "sr": {
                    "support": [
                        {"level": s["price_level"], "strength": s["strength"]}
                        for s in sr.get("support", [])
                    ],
                    "resistance": [
                        {"level": r["price_level"], "strength": r["strength"]}
            ...
            st.download_button(
                ...
                file_name="forex_analysis_v2.json",
...
"""
    ...
    features = [
        (
            ...
            "H&S, Triangles, Wedges, Flags, Channels, Breakouts",
        ),
        (
            "🕯️",
            "17 Candlestick Patterns",
            "Doji, Hammer, Engulfing, Pin Bar, Morning/Evening Star",
        ),
        (
            "📈",
            "Fibonacci Engine",
            "Retracements, Extensions, Golden Zone, Trade Zones",
        ),
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        '<p style="text-align:center;color:#6677aa;">⬆️ Upload a chart image using the sidebar to get started</p>',
        unsafe_allow_html=True,
    )
