...
def main():
    ...
    structure = (
        StructureAnalyzer().analyze(all_points, img.shape[0]) if all_points else {}
    )
    ...
    risk = RiskManager().calculate(
        sltp, sr, structure, img.shape[0], 10000, 2.0, "EURUSD"
    )
    annotated = Visualizer().create_full_overlay(
        img, patterns, sr, structure, sltp, regime
    )
    ...
    passed += sum(
        [
            ...
            validate_keys(
                "StructureAnalyzer",
                structure,
                ["price_series", "swing_highs", "swing_lows", "trend_direction"],
            ),
            validate_keys("SRDetector", sr, ["support", "resistance"]),
            validate_keys("FibonacciCalculator", fib, ["retracements", "extensions"]),
            validate_keys(
                "RegimeClassifier", regime, ["regime", "confidence", "indicators"]
            ),
            validate_keys(
                "SLTPCalculator",
                sltp,
                ["bias", "scenarios", "best_scenario", "current_price"],
            ),
            validate_keys(
                "ConfluenceEngine",
                confluence,
                ["master", "trade_plan", "signals", "bull_score", "bear_score"],
            ),
            validate_keys(
                "RiskManager",
                risk,
                ["best_scenario_risk", "risk_amount", "risk_of_ruin"],
            ),
            validate_keys(
                "LiquidityDetector",
                liquidity,
                ["buy_side_liquidity", "sell_side_liquidity", "summary"],
            ),
            validate_keys(
                "SessionAnalyzer", session, ["inferred_session", "volatility"]
            ),
            validate_keys("Visualizer", {"annotated": annotated}, ["annotated"]),
            (
                validate_keys(
                    "SLTP.best_scenario", sltp.get("best_scenario") or {}, ["direction"]
                )
                if sltp.get("best_scenario")
                else (
                    print("  ⚠️ SLTP.best_scenario: None (expected for weak signals)"),
                    True,
                )[1]
            ),
        ]
    ...
    ml_result = MLEnsemble().train_and_predict(
        fv, patterns, structure, regime, confluence
    )
    ad_result = MLAnomalyDetector().detect(fv, ps)
    wf_result = WalkForwardValidator().validate(
        fv, patterns, structure, confluence, n_windows=3
    )
    cal_result = CalibrationEngine().calibrate_and_predict(
        confluence, patterns, structure, regime, fv
    )
    sv_result = StatisticalValidator().full_probability_audit(
        ps, patterns, structure, confluence, n_simulations=200, n_bootstrap=200
    )
    meta_result = MetaLearner().predict_master(
        fv, ml_result, ad_result, cal_result, sv_result, wf_result, confluence
    )
    ...
    passed += sum(
        [
            validate_keys(
                "FeatureEngineer",
                feat_result,
                ["feature_vector", "features", "n_features", "feature_names"],
            ),
            validate_keys(
                "MLEnsemble",
                ml_result,
            ...
            validate_keys(
                ...
                wf_result,
                ["overall_metrics", "window_results", "overfitting_check"],
            ),
            validate_keys(
                "CalibrationEngine",
                cal_result,
                ["main_confluence", "calibration_quality"],
            ),
            validate_keys(
                "CalibrationEngine.main_confluence",
                cal_result.get("main_confluence", {}),
                [
                    "raw_heuristic",
                    "platt_calibrated",
                    "best_calibrated",
    ...
    bt_result = RealBacktester(db).backtest_current(
        patterns, confluence_grade, regime_name, session_name, "EURUSD"
    )
    kelly_result = RealKellyCalculator(db).calculate(
        "continuation_pattern", 10000, 50, "EURUSD"
    )
    ...
    passed += sum(
        [
            ...
            validate_keys(
                "RealBacktester", bt_result, ["pattern_backtests", "overall_verdict"]
            ),
            validate_keys(
                "RealKellyCalculator",
                kelly_result,
